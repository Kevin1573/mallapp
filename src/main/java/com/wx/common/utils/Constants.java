package com.wx.common.utils;

public class Constants {

    /**
     * 商户号
     */
    public static String MERCHANT_ID = "1697880258";

    /**
     * 商户API私钥路径
     */
//    public static String PRIVATE_KEY_PATH = "/Users/gzj/1657722877_20231101_cert/apiclient_key.pem";
    public static String PRIVATE_KEY_PATH = "/root/wx/apiclient_key.pem";

    /**
     * 商户证书序列号
     */
    public static String MERCHANT_SERIAL_NUMBER = "29E86573DF423E1924178D4844535D077BB3F37F";

    /**
     * 商户APIV3密钥
     */
    public static String API_V3_KEY = "b8a3dd1acb944ae9e1f13c2ca600b77D";


    public static String APP_ID = "wx16e16215491e8d5b";

    public static String APP_SECRET = "ccbcd061e9b448b1cc21c6d3a86a6033";

    public static String WX_LOGIN_URL = "https://api.weixin.qq.com/sns/jscode2session";

    public static final String ACCESS_KEY_ID = "LTAI5t7qG37bAV871WYektg2";

    public static final String ACCESS_KEY_SECRET = "ZpU5QYEo5Tbuy7f2obPKv6OFQm0wY2";

    public static final String ROLE_ARN = "acs:ram::1505343402756689:role/adminoss";

    public static final String ROLE_SESSION_NAME = "admin";

    public static final String ENDPOINT = "sts.cn-hangzhou.aliyuncs.com";

    public static final String REGION = "oss-cn-hangzhou";


    public static final String DEFAULT_HEAD = "https://0563headurl.oss-cn-hangzhou.aliyuncs.com/default/pic.webp";

}
